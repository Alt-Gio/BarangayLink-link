...
export function TeamTasks() {
  ...
  const canDelete = hasPermission(MODULE_PERMISSIONS.TASK_MANAGEMENT.DELETE);
          ...
          {task.assignees.length > 0 ? (
            <div className="flex flex-wrap gap-1">
              {task.assignees.map((assignee, index) => (
                <span
                  key={assignee.id}
                  className="px-2 py-1 bg-blue-900/20 text-blue-300 text-xs rounded-full border border-blue-700"
                >
                  {assignee.name}
                </span>
              ))}
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-gray-100">Team Tasks</h1>
          <p className="text-gray-400">Manage and track all team tasks and assignments</p>
        </div>
        <div className="flex items-center gap-2">
          {canCreate && (
            <Link 
      ...
      <div className="bg-blue-900/20 border border-blue-700 rounded-xl p-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-sm font-medium text-blue-300">Team Management Access</h3>
            <p className="text-xs text-blue-400">
              {canEdit ? 'Full access - can edit and manage all tasks' : 
               'View access - can view team tasks only'}
            </p>
          </div>
        </div>
      </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              {loading ? (
                ...
                <p className="text-2xl font-bold text-gray-100">{stats?.teamMembers || 0}</p>
              )}
